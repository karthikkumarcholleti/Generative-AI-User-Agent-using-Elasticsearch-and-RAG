...
class IdentityResolver:
    ...
    def build(self, adt_dir: str, ccda_dir: str, oru_dir: str):
        ...
        for dirpath, source_type in source_dirs:
            ...
            for fn in os.listdir(dirpath):
                ...
                self._records.append(
                    (given, family, pid, hospital, source_type, dob)
                )
        ...
        for given, family, pid, hospital, source_type, dob in self._records:
            if source_type != "ccda":
                continue
        ...
        # Step 2b — All remaining patients (Rules 2-5): one UUID per unique name
        for given, family, pid, hospital, source_type, dob in self._records:
            name_key = (given.lower(), family.lower())
            ...
            # Confidence: synthetic → 'synthetic', named with DOB → 'name_dob', else → 'name_only'
            if _is_synthetic(given, family):
                self.eid_to_confidence[eid] = "synthetic"
            else:
                self.eid_to_confidence[eid] = "name_dob" if dob else "name_only"

        # Step 2c — Update confidence for named patients whose DOB appears later
        dob_by_name: dict[tuple, bool] = defaultdict(bool)
        for given, family, pid, hospital, source_type, dob in self._records:
            if dob and not _is_synthetic(given, family):
                dob_by_name[(given.lower(), family.lower())] = True
        for name_key, has_dob in dob_by_name.items():
            eid = self.name_to_eid.get(name_key)
            if eid and self.eid_to_confidence.get(eid) == "name_only" and has_dob:
                self.eid_to_confidence[eid] = "name_dob"
        ...
        named   = sum(1 for e in self.eid_to_confidence.values() if e in ("enterprise_id","name_dob","name_only"))
        ...
        print(f"    named (no enterprise ID)  : {named - ent_ids:,}")
